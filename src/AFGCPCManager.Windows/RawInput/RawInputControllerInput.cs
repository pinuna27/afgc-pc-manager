...
public sealed class RawInputControllerInput : IRawControllerInput
{
    ...
    public async IAsyncEnumerable<RawControllerReport> ReadReportsAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var report in _reports.Reader.ReadAllAsync(cancellationToken)
    ...
    public ValueTask DisposeAsync()
    {
        ...
        SharedHost.Value.Unsubscribe(_subscription);
    ...
    private sealed class RawInputHost
    {
        ...
        public RawInputHost()
        {
            var thread = new Thread(MessageThread)
            {
                IsBackground = true,
                Name = "AFGC Raw Input Router"
            };
        ...
        public Guid Subscribe(HashSet<string>? paths, Action<byte[]> publish)
        {
            ...
            lock (_gate)
                _subscriptions.Add(id, new(paths, publish));
            return id;
        }
        ...
        private void MessageThread()
        {
            try
            {
                using var window = new RawInputWindow(Route);
                _ready.TrySetResult();
                Application.Run();
            }
            catch (Exception ex)
        ...
        private void Route(string path, byte[] report)
        {
            ...
            lock (_gate)
                targets = _subscriptions.Values.ToArray();
            foreach (Subscription target in targets)
            {
                if (target.Paths is null || target.Paths.Contains(path))
    ...
    private sealed class RawInputWindow : NativeWindow, IDisposable
    {
        ...
        {
            _publish = publish;
            CreateHandle(new CreateParams { Caption = "AFGC PC Manager Raw Input Router" });
            ...
            if (!RegisterRawInputDevices(devices, (uint)devices.Length,
                ...
                throw new Win32Exception(Marshal.GetLastWin32Error(),
                    "Could not register for raw controller input.");
        }
        protected override void WndProc(ref Message message)
        {
            if (message.Msg == WmInput)
                Capture(message.LParam);
        ...
        private unsafe void Capture(nint inputHandle)
        {
            ...
            if (GetRawInputData(inputHandle, RidInput, null, ref size, headerSize) != 0
                || size < headerSize + 8)
                return;
            byte[] buffer = new byte[size];
            fixed (byte* pointer = buffer)
            {
                if (GetRawInputData(inputHandle, RidInput, pointer, ref size, headerSize)
                    != size)
                    return;
                RawInputHeader header = Marshal.PtrToStructure<RawInputHeader>(
                    (nint)pointer);
                string path = GetDeviceName(header.Device);
                if (!FireDevicePathMatcher.IsMatch(path))
                    return;
                int offset = checked((int)headerSize);
                uint reportSize = *(uint*)(pointer + offset);
                uint reportCount = *(uint*)(pointer + offset + 4);
                foreach (byte[] report in RawHidReportSplitter.Split(
                             buffer.AsSpan(offset + 8), reportSize, reportCount))
        ...
        {
            uint length = 0;
            uint sizeResult = GetRawInputDeviceInfo(
                device, RidiDeviceName, null, ref length);
            if (sizeResult == uint.MaxValue || length == 0)
                return string.Empty;
            var value = new StringBuilder((int)length);
            return GetRawInputDeviceInfo(device, RidiDeviceName, value, ref length)
                == uint.MaxValue
                    ? string.Empty
                    : value.ToString();
        }
        ...
        private readonly record struct RawInputDevice(
            ushort UsagePage, ushort Usage, uint Flags, nint Target);
