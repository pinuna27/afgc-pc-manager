...
public sealed class ReleaseManifestVerifier(string publicKeyPem)
{
    ...
    private static string? Validate(ReleaseManifest? value)
    {
        ...
        if (string.IsNullOrWhiteSpace(value.Version)
            || !Version.TryParse(value.Version.TrimStart('v', 'V'), out _)) return "The release version is invalid.";
        if (!string.Equals(value.Architecture, "x64", StringComparison.OrdinalIgnoreCase)) return "The release architecture is unsupported.";
        if (value.PublishedAtUtc == default) return "The release publication time is invalid.";
        ...
        foreach (DependencyRelease dependency in new[] { value.VJoy, value.HidHide }.OfType<DependencyRelease>())
            if (!IsRepository(dependency.Repository) || !IsRepositoryPart(dependency.ReleaseTag)
                ...
                || string.IsNullOrWhiteSpace(dependency.ExpectedPublisher)) return "A dependency release entry is invalid.";
        return null;
    }
    ...
    private static bool IsRepository(string value) { if (value is null) return false; string[] parts = value.Split('/'); return parts.Length == 2 && parts.All(IsRepositoryPart); }
