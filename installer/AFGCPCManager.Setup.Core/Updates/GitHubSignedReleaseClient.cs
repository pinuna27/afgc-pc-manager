...
public sealed class GitHubSignedReleaseClient(HttpClient client, ReleaseManifestVerifier verifier)
{
    private const int MaximumManifestBytes = 1024 * 1024, MaximumSignatureBytes = 4096;
    public async Task<VerifiedRelease> GetLatestAsync(string owner, string repository, CancellationToken cancellationToken = default)
    {
        ValidateRepositoryPart(owner); ValidateRepositoryPart(repository);
        using var request = Request($"https://api.github.com/repos/{owner}/{repository}/releases/latest");
    ...
    {
        var expected = release.Manifest.Assets.SingleOrDefault(x => x.Name.Equals(assetName, StringComparison.OrdinalIgnoreCase)) ?? throw new InvalidDataException("The requested asset is not in the signed manifest.");
        if (!release.AssetDownloads.TryGetValue(expected.Name, out Uri? uri)) throw new InvalidDataException("The requested release asset is unavailable.");
        Directory.CreateDirectory(destinationDirectory); string destination = Path.Combine(destinationDirectory, expected.Name), temporary = destination + ".download";
        using var request = Request(uri.AbsoluteUri); using HttpResponseMessage response = await client.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, cancellationToken); response.EnsureSuccessStatusCode();
        if (response.Content.Headers.ContentLength is long declared && declared != expected.Size) throw new InvalidDataException("The release asset size does not match its signed manifest.");
        await using Stream source = await response.Content.ReadAsStreamAsync(cancellationToken); await using FileStream target = new(temporary, FileMode.Create, FileAccess.Write, FileShare.None);
        byte[] buffer = new byte[81920]; long total = 0; int read;
        try
        {
            while ((read = await source.ReadAsync(buffer, cancellationToken)) > 0) { total += read; if (total > expected.Size) throw new InvalidDataException("The release asset exceeded its signed size."); await target.WriteAsync(buffer.AsMemory(0, read), cancellationToken); progress?.Report((double)total / expected.Size); }
            await target.FlushAsync(cancellationToken); target.Close(); if (total != expected.Size) throw new InvalidDataException("The release asset was truncated.");
    ...
    public async Task<string> DownloadDependencyAsync(DependencyRelease dependency, string destinationDirectory, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        if (repository.Length != 2) throw new InvalidDataException("The signed dependency repository is invalid.");
        ...
        return await new DependencyDownloader(client, new PackageVerifier()).DownloadVerifiedAsync(entry, destinationDirectory, progress, cancellationToken);
    }
    private async Task<byte[]> DownloadSmallAsync(Uri uri, int maximum, CancellationToken cancellationToken)
    {
        using var request = Request(uri.AbsoluteUri); using HttpResponseMessage response = await client.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, cancellationToken); response.EnsureSuccessStatusCode();
        if (response.Content.Headers.ContentLength > maximum) throw new InvalidDataException("A release metadata asset is too large.");
        await using Stream stream = await response.Content.ReadAsStreamAsync(cancellationToken); using var memory = new MemoryStream(); byte[] buffer = new byte[8192]; int read;
        while ((read = await stream.ReadAsync(buffer, cancellationToken)) > 0) { if (memory.Length + read > maximum) throw new InvalidDataException("A release metadata asset is too large."); memory.Write(buffer, 0, read); }
