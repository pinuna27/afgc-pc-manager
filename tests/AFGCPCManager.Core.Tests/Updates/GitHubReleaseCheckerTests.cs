...
public sealed class GitHubReleaseCheckerTests
{
    ...
    public async Task AcceptsNewerStableRelease()
    {
        ...
        var result = await new GitHubReleaseChecker(client).CheckAsync(
            new(ReleaseComponent.AfgcPcManager, "pinuna27", "afgc-pc-manager", new(1, 1, 0)),
    ...
    public async Task RejectsPrereleaseResponse()
    {
        ...
        var result = await new GitHubReleaseChecker(client).CheckAsync(
            new(ReleaseComponent.VJoy, "a", "b", new(1, 0)),
            TestContext.Current.CancellationToken);
